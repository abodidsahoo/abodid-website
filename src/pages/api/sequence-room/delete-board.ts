...
export const POST: APIRoute = async ({ request }) => {
    ...
    try {
        ...
        const { data: items, error: itemError } = await authorization.supabase
            ...
            .select('asset_id,user_photo_assets(id,cloudflare_key,is_library_asset)')
            .eq('board_id', boardId);
        ...
        return sequenceRoomJson({ deleted: true });
    } catch (error) {
        console.error('Sequence Room deletion failed:', error);
        return sequenceRoomJson({ error: 'The board could not be deleted.' }, 500);
    }
};
